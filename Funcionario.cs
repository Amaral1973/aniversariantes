...
namespace AppEmpresa
{
    public class Funcionario
    {
        ...
        SqlConnection con = new SqlConnection("Data Source=(LocalDB)\\MSSQLLocalDB;AttachDbFilename=C:\\Programas\\AppEmpresa\\DbEmpresa.mdf;Integrated Security=True");
        ...
        public bool RegistroRepetido(string nome, string matricula)
        {
            string sql = "SELECT * FROM Funcionario WHERE nome='" + nome + "' AND matricula='"+matricula+"'";
            if (con.State == ConnectionState.Open)
            {
                con.Close();
            }
